...
- **Запись (Recording)** - пофреймовая запись в SQLite. Архивируется каждый фрейм. Декодированные SPN лежат как gzip-сжатый JSON, у каждого свой timestamp. Поддерживаются kernel-таймстемпы из `candump -t a` / `-t A`
...
- **Manual-режим / reverse-engineering** - построение собственных SPN поверх живого CAN ID без записи в J1939 DA. Выбираешь любой диапазон байт/бит в payload'е, задаёшь resolution/offset/unit/endianness и смотришь,
  ...
  Custom SPN видны во вкладке `charts`, в NDJSON в headless-режиме и в диалоге экспорта параметров
- **Real-time TUI** - обновление UI на 30 fps
...
**Regex search** - фильтр по CAN ID / PGN-метке, по description
![regex_search](pics/regex_search.png)

**Parameter export** - выбор SPN по нескольким CAN ID для экспорта в JSON (полезно для отчетов)
...
- CMake >= 3.13
- Ninja
- Системные библиотеки: boost (signals2, spirit, phoenix, regex, sqlite3, zlib)
