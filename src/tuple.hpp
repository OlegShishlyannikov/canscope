...
constexpr Func for_each_impl(Tuple &&t, Func &&f, std::index_sequence<I...> is) {
...
auto transform_impl(std::tuple<Ts...> const &inputs, Function function, std::index_sequence<Is...> is) {
  return std::tuple<std::result_of_t<Function(Ts)>...>{function(std::get<Is>(inputs))...};
}

template <typename... T, std::size_t... i> auto subtuple(const std::tuple<T...> &t, std::index_sequence<i...>) {
...
template <std::size_t I, typename... Tuples> zip_tuple_at_index_t<I, Tuples...> zip_tuple_at_index(Tuples &&...tuples) {
...
template <typename Tuple, typename Predicate> bool none_of(Tuple &&tuple, Predicate pred) {
  return find_if(tuple, pred) == std::tuple_size<std::decay_t<Tuple>>::value;
}
template <typename Tuple, typename Predicate> bool any_of(Tuple &&tuple, Predicate pred) {
...
template <int trim, typename... T> auto subtuple(const std::tuple<T...> &t) {
  return detail::subtuple(t, std::make_index_sequence<sizeof...(T) - trim>());
}
...
auto tuple_zip(Head &&head, Tail &&...tail) {
  return detail::tuple_zip_impl<Head, Tail...>(std::forward<Head>(head), std::forward<Tail>(tail)...,
                                               std::make_index_sequence<std::tuple_size_v<std::decay_t<Head>>>());
}
}; // namespace tp
